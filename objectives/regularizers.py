...
def compute_jacobian_symmetric_loss(input_coords, output):
    """Compute the jacobian regularization loss."""
    ...
    loss = resdet - 1 + torch.ones(resdet.shape).cuda()/resdet - 1
...
def compute_jacobian_matrix(input_coords, output, add_identity=True):
    ...
    jacobian_matrix = torch.zeros(input_coords.shape[0], 3, 3).cuda()
    for i in range(3):
...
def compute_ptv_loss(input_coords, output, alpha=1.0):
    """Compute the pseudo Total Variation (pTV) regularization loss."""
    
    # Compute the gradient of the output with respect to the input coordinates
    grad = gradient(input_coords, output)
    
    # Compute the pTV loss
    ptv_loss = torch.norm(grad, p=1, dim=-1).mean()
    
    return alpha * ptv_loss